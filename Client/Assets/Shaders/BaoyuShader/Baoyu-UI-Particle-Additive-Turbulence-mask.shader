...
Shader "Baoyu/UI/Particles/Additive-Turbulence-mask"{
	Properties 
	{
	...
	//	ColorMask RGB
		...
		SubShader {
			Pass {
				...
				{
					bool a = i.worldPos.x >= _ClipRange.x;
					a = a * (i.worldPos.x <= _ClipRange.z);
					a = a * (i.worldPos.y >= _ClipRange.y);
					a = a * (i.worldPos.y <= _ClipRange.w);

					fixed4 MainCol = _TintColor * tex2D(_MainTex,i.mainTexUV + float2(0,_Time.x*_Speed00));
					fixed4 BlendCol01= _Blend_Color_01 * tex2D(_Blend_Texture_01,i.blendTexUV1+float2(0,_Time.x*_Speed01));
...
//					float4 MaskCol = tex2D(_MaskTex,i.texcoord);
					...
					float4 Sampled2D0 = tex2D(_MaskTex,i.uv_MaskTex.xy);
					col = _LightenMain * col * i.color* Sampled2D0;
					col.a = col.a * a;
					return col;
				}
				ENDCG 

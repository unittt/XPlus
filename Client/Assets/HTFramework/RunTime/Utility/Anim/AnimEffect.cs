...
namespace HT.Framework
{
    ...
    public class AnimEffect : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        [SerializeField] private int effectLen = 0;
        public AnimEffectInfo[] effectArray;

        //序列化用数据
        [SerializeField] List<GameObject> effectSerial1;
        [SerializeField] List<string> effectSerial2;

        [SerializeField] private int soundLen = 0;
        public AnimEffectInfo[] soundArray;

        //序列化用数据
        [SerializeField] List<string> soundSerial1;
        [SerializeField] List<int> soundSerial2;

        public int EffectLength
        {
            get { return effectLen; }
            set
            {
        ...
        public void OnAfterDeserialize()
        {
            if (effectSerial1 != null && effectSerial2 != null)
            {
                ...
                {
                    Log.Error("动作绑定配置错误啦（特效绑定点挂点问题）" + gameObject.name);
                    return;
                }

                if (effectSerial2.Count < EffectLength)
                {
                    Log.Error("动作绑定配置错误啦（特效绑定路径问题）" + gameObject.name);
                    return;
                }
            ...
            if (soundSerial1 != null && soundSerial2 != null)
            {
                ...
                {
                    Log.Error("动作绑定配置错误啦（音效绑定路径问题）" + gameObject.name);
                    return;
                }

                if (soundSerial2.Count < SoundLength)
                {
                    Log.Error("动作绑定配置错误啦（音效绑定偏移问题）" + gameObject.name);
                    return;
                }

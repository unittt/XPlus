...
namespace HT.Framework
{
    ...
    ///YooAsset资源管理器助手
    /// </summary>
    internal sealed partial class YooAssetResourceHelper : IResourceHelper
    {
        ...
        private async UniTask<RawFileHandle> LoadRawFileAsync(string location)
        {
            var beginTime = Time.realtimeSinceStartup;
            //单线加载，如果其他地方在加载资源，则等待
            if (_isLoading )
        ...
        /// <returns></returns>
        public async UniTask<Scene> LoadSceneAsync(string location,HTFAction<float> onLoading = null, LoadSceneMode sceneMode = LoadSceneMode.Single, bool suspendLoad = false)
        {
            var beginTime = Time.realtimeSinceStartup;
            //单线加载，如果其他地方在加载资源，则等待
            if (_isLoading )
